...
export default function BannerAdmin() {
  ...
  const loadBanners = () => {
    fetch(API_BASE + "/banners", { credentials: "include" })
      .then((res) => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch(API_BASE + "/admin/banner/upload", {
        method: "POST",
        body: form,
        credentials: "include", // ВАЖНО: кука суперпользователя
      });

      if (res.status === 401) {
        alert("Сессия устарела. Войдите снова.");
        return;
      }
    ...
    <div style={{ marginBottom: 30 }}>
      ...
      >
        <input
          ...
          accept="image/*"
          required
          onChange={(e) => setNewFile(e.target.files && e.target.files[0])}
          disabled={uploading}
        />
        <input
          type="text"
